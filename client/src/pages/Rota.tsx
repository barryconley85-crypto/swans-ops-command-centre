...
import { CalendarClock, ChevronLeft, ChevronRight, CircleAlert, Plus, RotateCcw, Trash2, UserRoundCheck } from "lucide-react";
...
export default function Rota() {
  ...
  const moveWeek = (offset: number) => { const changed = addDays(new Date(`${weekStart}T12:00:00`), offset * 7); const key = localDateKey(changed); setWeekStart(key); setForm(current => ({ ...current, workDate: key })); };
  const submit = () => { if (!form.teamMemberId) return toast.error("Choose the team member who owns this shift."); create.mutate({ workDate: form.workDate, teamMemberId: Number(form.teamMemberId), assignmentType: form.assignmentType as "early" | "core" | "late" | "on_call" | "leave" | "unavailable", startTime: ["leave", "unavailable", "on_call"].includes(form.assignmentType) ? undefined : form.startTime, endTime: ["leave", "unavailable", "on_call"].includes(form.assignmentType) ? undefined : form.endTime, note: form.note || undefined }); };
  ...
  return <div className="mx-auto max-w-[1500px] pb-10"><PageHeader eyebrow={`Week commencing ${format(days[0]!, "d MMMM")}`} title="Rota & on-call" description="Plan cover, make availability constraints explicit, and keep the on-call duty fair and visible." actions={<><div className="flex h-10 items-center rounded-xl border border-[#DFE6E1] bg-white p-1 shadow-sm"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => moveWeek(-1)}><ChevronLeft className="h-4 w-4" /></Button><span className="min-w-36 px-2 text-center text-sm font-semibold text-[#34413C]">{format(days[0]!, "d MMM")} – {format(days[6]!, "d MMM")}</span><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => moveWeek(1)}><ChevronRight className="h-4 w-4" /></Button></div>{isManager ? <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="bg-[#1D5C63] hover:bg-[#164B50]"><Plus className="mr-2 h-4 w-4" />Add cover</Button></DialogTrigger><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Add rota assignment</DialogTitle><DialogDescription>Use shift, on-call, leave or unavailable assignments so that cover gaps are evident.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Date</Label><Input type="date" value={form.workDate} onChange={event => setForm({ ...form, workDate: event.target.value })} /></div><div className="space-y-2"><Label>Team member</Label><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.teamMemberId} onChange={event => setForm({ ...form, teamMemberId: event.target.value })}><option value="">Choose…</option>{activeMembers.map(member => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select></div></div><div className="space-y-2"><Label>Assignment</Label><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.assignmentType} onChange={event => setForm({ ...form, assignmentType: event.target.value })}><option value="early">Early shift</option><option value="core">Core shift</option><option value="late">Late shift</option><option value="on_call">On-call lead</option><option value="leave">Leave</option><option value="unavailable">Unavailable</option></select></div>{!["leave", "unavailable", "on_call"].includes(form.assignmentType) ? <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Start</Label><Input type="time" value={form.startTime} onChange={event => setForm({ ...form, startTime: event.target.value })} /></div><div className="space-y-2"><Label>End</Label><Input type="time" value={form.endTime} onChange={event => setForm({ ...form, endTime: event.target.value })} /></div></div> : null}<div className="space-y-2"><Label>Cover note</Label><Input value={form.note} onChange={event => setForm({ ...form, note: event.target.value })} placeholder="Optional context or location" /></div></div><DialogFooter><Button disabled={create.isPending} onClick={submit} className="bg-[#1D5C63] hover:bg-[#164B50]">Save assignment</Button></DialogFooter></DialogContent></Dialog> : null}</>} />
        ...
        {activeMembers.length ? activeMembers.map(member => (
          <div key={member.id} className="grid grid-cols-[200px_repeat(7,minmax(100px,1fr))] border-b border-[#EFF2F0] last:border-0">
            ...
            {days.map(day => {
              ...
              return <div key={day.toISOString()} className={`min-h-[86px] border-l border-[#EFF2F0] p-2 ${localDateKey(day) === localDateKey() ? "bg-[#FBFDFC]" : ""}`}>
                {assignments.map(item => <div key={item.id} className={`group relative mb-1 rounded-lg border px-2 py-1.5 text-[10px] font-semibold capitalize ${assignmentStyle[item.assignmentType]}`}><p>{item.assignmentType.replace("_", " ")}</p>{item.startTime ? <p className="mt-0.5 text-[9px] font-medium opacity-75">{item.startTime}–{item.endTime}</p> : null}{isManager ? <button onClick={() => { if (window.confirm("Remove this rota assignment? This action cannot be undone.")) remove.mutate({ id: item.id }); }} className="absolute -right-1.5 -top-1.5 hidden h-4 w-4 items-center justify-center rounded-full bg-[#3E4B45] text-white group-hover:flex"><Trash2 className="h-2.5 w-2.5" /></button> : null}</div>)}
              </div>;
            })}
          </div>
