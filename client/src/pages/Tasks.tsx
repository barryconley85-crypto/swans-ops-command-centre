...
import { Check, ChevronLeft, ChevronRight, CircleAlert, ClipboardCheck, Clock3, ExternalLink, FileText, Link, MessageSquareText, Pencil, Plus, RotateCcw, UsersRound, Save, Sparkles, Trash2, X } from "lucide-react";
...
type TaskRow = {
  ...
  status: "pending" | "in_progress" | "blocked" | "missed" | "complete";
  dueAt: number | null;
  ...
  assistanceTeamMemberIds?: number[];
  assistanceRequestedAt?: number | null;
  assistanceRequestedByName?: string | null;
...
function TaskDetail({ task, selectedDate, isManager = false, members = [] }: { task: TaskRow; selectedDate: string; isManager?: boolean; members?: any[] }) {
  ...
  const [missedForm, setMissedForm] = useState({ reason: "", followUp: "" });
  const [assistanceOpen, setAssistanceOpen] = useState(false);
  const [assistanceIds, setAssistanceIds] = useState<string[]>([]);
  ...
  const assistanceMutation = trpc.operations.tasks.requestAssistance.useMutation({ onSuccess: async () => { setAssistanceOpen(false); setAssistanceIds([]); await utils.operations.tasks.list.invalidate({ date: selectedDate }); toast.success("Assistance requested."); }, onError: error => toast.error(error.message) });
  ...
  const visibilityMutation = trpc.operations.tasks.setVisibility.useMutation({
    onSuccess: async () => { await utils.operations.tasks.list.invalidate({ date: selectedDate }); setOpen(false); toast.success("Task visibility updated."); },
    onError: error => toast.error(error.message),
  });
  ...
  const unblockMutation = trpc.operations.tasks.unblock.useMutation({ onSuccess: async () => { await Promise.all([utils.operations.tasks.list.invalidate({ date: selectedDate }), activity.refetch()]); toast.success("Block removed and task returned to pending work."); }, onError: error => toast.error(error.message) });
  const missedMutation = trpc.operations.tasks.recordMissed.useMutation({ onSuccess: async () => { setMissedOpen(false); setMissedForm({ reason: "", followUp: "" }); await Promise.all([utils.operations.tasks.list.invalidate({ date: selectedDate }), activity.refetch()]); toast.success("Missed task recorded and added to the action log."); }, onError: error => toast.error(error.message) });
  ...
  const submitAssistance = () => { if (!assistanceIds.length) return toast.error("Choose at least one colleague to assist."); assistanceMutation.mutate({ taskId: task.id, teamMemberIds: assistanceIds.map(Number) }); };
  ...
  const submitMissed = () => { if (!missedForm.reason.trim() || !missedForm.followUp.trim()) return toast.error("Add why the task was missed and what needs to happen next."); missedMutation.mutate({ taskId: task.id, reason: missedForm.reason, followUp: missedForm.followUp }); };
  const removeTask = () => { const reason = window.prompt(`Why are you deleting “${task.title}”? This reason will be logged.`); if (!reason?.trim()) return toast.error("A deletion reason is required; the task was not deleted."); if (window.confirm(`Delete “${task.title}”? This cannot be undone.

Reason: ${reason.trim()}`)) deleteMutation.mutate({ taskId: task.id, reason: reason.trim() }); };
  ...
  return (
    <Dialog open={open} onOpenChange={value => { setOpen(value); if (!value) setEditing(false); }}>
      ...
      <Dialog open={assistanceOpen} onOpenChange={setAssistanceOpen}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Request assistance</DialogTitle><DialogDescription>{task.assignee?.displayName || "The task owner"} remains accountable, while selected colleagues are added as supporting helpers.</DialogDescription></DialogHeader><div className="space-y-2 py-2"><Label>Ask colleagues to assist</Label><div className="max-h-48 space-y-2 overflow-y-auto rounded-lg border border-[#E1E7E3] bg-white p-3">{teamMembers.filter(member => member.status === "active" && member.id !== task.assignedTeamMemberId).map(member => <label key={member.id} className="flex items-center gap-2 text-sm text-[#405249]"><input type="checkbox" checked={assistanceIds.includes(String(member.id))} onChange={event => setAssistanceIds(current => event.target.checked ? [...current, String(member.id)] : current.filter(id => id !== String(member.id)))} />{member.displayName}</label>)}</div></div><DialogFooter><Button variant="outline" onClick={() => setAssistanceOpen(false)}>Cancel</Button><Button onClick={submitAssistance} disabled={assistanceMutation.isPending} className="bg-[#1D5C63] hover:bg-[#164B50]">{assistanceMutation.isPending ? "Sending…" : "Request assistance"}</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={missedOpen} onOpenChange={setMissedOpen}><DialogContent className="max-w-xl"><DialogHeader><DialogTitle>Record missed task</DialogTitle><DialogDescription>This records that the work was not completed, keeps the original owner visible, and creates an audit entry for follow-up.</DialogDescription></DialogHeader><div className="space-y-3 py-2"><div><Label>Why was this task missed?</Label><Textarea value={missedForm.reason} onChange={event => setMissedForm({ ...missedForm, reason: event.target.value })} className="mt-1.5 min-h-20" placeholder="No confirmation received before the deadline…" /></div><div><Label>What needs to happen now?</Label><Textarea value={missedForm.followUp} onChange={event => setMissedForm({ ...missedForm, followUp: event.target.value })} className="mt-1.5 min-h-20" placeholder="Reassign the check and confirm completion by 14:00…" /></div></div><DialogFooter><Button variant="outline" onClick={() => setMissedOpen(false)}>Cancel</Button><Button onClick={submitMissed} disabled={missedMutation.isPending} className="bg-[#A25E16] hover:bg-[#874D12]">{missedMutation.isPending ? "Recording…" : "Record missed task"}</Button></DialogFooter></DialogContent></Dialog>
...
<DialogContent className="max-w-xl border-0 bg-[#FCFCFA] p-0 shadow-2xl">
        <div className="border-b border-[#E5E9E6] px-6 pb-5 pt-6">
          <div className="mb-4 flex items-start justify-between gap-3"><div className="flex items-center gap-2"><TaskStatusBadge status={task.status} /><Badge className={`border-0 text-[10px] uppercase tracking-[0.12em] ${priorityStyle[task.priority]}`}>{task.priority}</Badge></div>{canManageTasks && !editing ? <div className="flex flex-wrap justify-end gap-1.5">{currentUser?.isSuperuser && task.status !== "complete" && task.status !== "missed" ? <Button size="sm" variant="outline" onClick={() => setMissedOpen(true)}><CircleAlert className="mr-1.5 h-3.5 w-3.5 text-[#A25E16]" />Record missed</Button> : null}<Button size="sm" variant="outline" onClick={() => { setAssistanceIds((task.assistanceTeamMemberIds || []).map(String)); setAssistanceOpen(true); }}><UsersRound className="mr-1.5 h-3.5 w-3.5" />Ask for assistance</Button><Button size="sm" variant="outline" onClick={() => { setDraft({ workDate: task.workDate, title: task.title, detail: task.detail || "", priority: task.priority, dueTime: task.dueAt ? format(new Date(task.dueAt), "HH:mm") : "", assignedTeamMemberId: task.assignedTeamMemberId ? String(task.assignedTeamMemberId) : "" }); setEditing(true); }}><Pencil className="mr-1.5 h-3.5 w-3.5" />Edit</Button>{task.status === "blocked" ? <Button size="sm" variant="outline" disabled={unblockMutation.isPending} onClick={() => { if (window.confirm("Remove the block and return this task to pending work? The prerequisite task will remain for tracking.")) unblockMutation.mutate({ taskId: task.id }); }}><RotateCcw className="mr-1.5 h-3.5 w-3.5" />Remove block</Button> : null}<Button size="sm" variant="outline" disabled={visibilityMutation.isPending} onClick={() => visibilityMutation.mutate({ taskId: task.id, isPersonal: !task.isPersonal })}>{task.isPersonal ? "Make shared" : "Make personal"}</Button></div> : null}</div>
        ...
        <div className="grid gap-5 px-6 py-5 sm:grid-cols-3">
          ...
          <div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#87918D]">Original date</p><p className="mt-1.5 text-sm font-medium text-[#28332F]">{task.originalWorkDate ? format(new Date(`${task.originalWorkDate}T12:00:00`), "d MMM yyyy") : "Not recorded"}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#87918D]">Due</p><p className="mt-1.5 text-sm font-medium text-[#28332F]">{compactTime(task.dueAt)}</p></div>
        ...
        {task.assistanceTeamMemberIds?.length ? <div className="mx-6 mb-3 rounded-xl border border-[#D8E7EF] bg-[#F1F8FB] px-4 py-3 text-sm text-[#2C6475]"><strong>Assisting:</strong> {task.assistanceTeamMemberIds.map(id => teamMembers.find(member => member.id === id)?.displayName).filter(Boolean).join(", ")}</div> : null}
        ...
        <div className="border-t border-[#E5E9E6] px-6 py-5">
          ...
          <div className="mt-4 flex justify-start"><Button size="sm" variant="outline" onClick={removeTask} disabled={deleteMutation.isPending} className="border-[#F0C9C3] text-[#A84237] hover:bg-[#FFF2F0]"><Trash2 className="mr-1.5 h-3.5 w-3.5" />Delete task</Button></div>
...
export default function Tasks() {
  ...
  const [templateForm, setTemplateForm] = useState({ name: "", description: "", defaultTime: "", items: "" });
  ...
  const templateMutation = trpc.operations.templates.create.useMutation({
    onSuccess: async () => { await templatesQuery.refetch(); setShowTemplateForm(false); setEditingTemplateId(null); setTemplateForm({ name: "", description: "", defaultTime: "", items: "" }); toast.success("Checklist template saved."); },
    onError: error => toast.error(error.message),
  });
  const templateUpdateMutation = trpc.operations.templates.update.useMutation({
    onSuccess: async () => { await templatesQuery.refetch(); setShowTemplateForm(false); setEditingTemplateId(null); setTemplateForm({ name: "", description: "", defaultTime: "", items: "" }); toast.success("Checklist template updated."); },
    onError: error => toast.error(error.message),
  });
  const templateRemoveMutation = trpc.operations.templates.remove.useMutation({
    onSuccess: async () => { await templatesQuery.refetch(); toast.success("Checklist template deleted."); },
    onError: error => toast.error(error.message),
  });
  ...
  const visibleTasks = showPersonal ? tasks.filter(task => task.isPersonal) : showMine && personalProfile ? tasks.filter(task => task.assignedTeamMemberId === personalProfile.id) : tasks;
  const latestCommentByTask = useMemo(() => {
    const latest = new Map<number, any>();
    (state.activities || []).filter((item: any) => item.action === "comment" && item.body).forEach((item: any) => {
      const current = latest.get(item.taskId);
      if (!current || (item.createdAt || 0) > (current.createdAt || 0)) latest.set(item.taskId, item);
    });
    return latest;
  }, [state.activities]);
  const grouped = useMemo(() => ({
    action: visibleTasks.filter(task => !["complete", "blocked", "missed"].includes(task.status)),
    blocked: visibleTasks.filter(task => task.status === "blocked"),
    missed: visibleTasks.filter(task => task.status === "missed"),
    complete: visibleTasks.filter(task => task.status === "complete"),
  }), [visibleTasks]);
  const visibleIssues = (state.issues || [])
    .filter((issue: any) => ["open", "monitoring"].includes(issue.status))
    .filter((issue: any) => !showMine || !personalProfile || issue.ownerTeamMemberId === personalProfile.id)
    .sort((a: any, b: any) => (b.impact === "critical" ? 4 : b.impact === "high" ? 3 : b.impact === "medium" ? 2 : 1) - (a.impact === "critical" ? 4 : a.impact === "high" ? 3 : a.impact === "medium" ? 2 : 1));
  ...
  const templateItemRows = templateForm.items ? templateForm.items.split("\n").map(line => { const trimmedLine = line.trim(); const match = trimmedLine.match(/^(\d{1,2}:\d{2})\s*[|\-]\s*(.+)$/); return { title: !trimmedLine ? "" : match ? match[2].trim() : line, dueTime: match ? match[1].padStart(5, "0") : "" }; }) : [];
  ...
  const submitTemplate = () => {
    ...
    if (templateForm.defaultTime && !/^([01]\d|2[0-3]):[0-5]\d$/.test(templateForm.defaultTime)) return toast.error("Choose a valid default time.");
    const payload = { name: templateForm.name.trim(), description: templateForm.description.trim() || undefined, defaultTime: templateForm.defaultTime || null, items };
    if (editingTemplateId) templateUpdateMutation.mutate({ id: editingTemplateId, ...payload });
    else templateMutation.mutate(payload);
  };
  ...
  return (
      ...
      <header className="mb-7 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#74817B]">Daily work control</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.035em] text-[#1B2723]">Tasks & checklists</h1><p className="mt-2 text-sm text-[#68746E]">Make every operational obligation visible, owned and evidenced.</p></div><div className="flex flex-wrap items-center gap-2"><div className="flex h-10 items-center rounded-xl border border-[#DFE6E1] bg-white p-1 shadow-sm"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => changeDay(-1)}><ChevronLeft className="h-4 w-4" /></Button><label className="flex h-8 items-center rounded-lg px-2 text-sm font-semibold text-[#34413C] hover:bg-[#F3F7F4]" title="Choose task date"><span className="sr-only">Choose task date</span><input type="date" value={selectedDate} onChange={event => { if (event.target.value) setSelectedDate(event.target.value); }} className="w-32 cursor-pointer border-0 bg-transparent text-center text-sm font-semibold text-[#34413C] outline-none" /></label><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => changeDay(1)}><ChevronRight className="h-4 w-4" /></Button></div>{personalProfile ? <><Button variant="outline" className={`h-10 border-[#D6E2DE] bg-white ${showPersonal ? "border-[#1D5C63] text-[#1D5C63]" : ""}`} onClick={() => { setShowPersonal(value => !value); setShowMine(false); }}>{showPersonal ? "Personal work on" : "Personal work"}</Button><Button variant="outline" className="h-10 border-[#D6E2DE] bg-white" onClick={() => { setShowMine(value => !value); setShowPersonal(false); }}>{showMine ? "All work" : "My work"}</Button></> : null}{isManager ? <><Button variant="outline" className="h-10 border-[#D6E2DE] bg-white" onClick={() => setShowWeeklyPlanner(true)}><ClipboardCheck className="mr-2 h-4 w-4" />Plan a week</Button><Dialog open={showTemplateForm} onOpenChange={setShowTemplateForm}><DialogTrigger asChild><Button variant="outline" className="h-10 border-[#D6E2DE] bg-white" onClick={() => { setEditingTemplateId(null); setTemplateForm({ name: "", description: "", defaultTime: "", items: "" }); }}><FileText className="mr-2 h-4 w-4" />New template</Button></DialogTrigger><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>{editingTemplateId ? "Edit checklist template" : "Create a reusable checklist"}</DialogTitle><DialogDescription>Use one item per line. Then use Plan a week to choose the days and named owners for each item.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="space-y-2"><Label>Checklist name</Label><Input value={templateForm.name} onChange={event => setTemplateForm({ ...templateForm, name: event.target.value })} placeholder="Morning control checks" /></div><div className="space-y-2"><Label>Default completion time</Label><p className="text-xs text-[#78857E]">Applied to every checklist item unless that item has its own time.</p><Input type="time" value={templateForm.defaultTime} onChange={event => setTemplateForm({ ...templateForm, defaultTime: event.target.value })} /></div><div className="space-y-2"><Label>Purpose</Label><Input value={templateForm.description} onChange={event => setTemplateForm({ ...templateForm, description: event.target.value })} placeholder="What does this make sure is ready?" /></div><div className="space-y-2"><div className="flex items-center justify-between"><div><Label>Checklist items</Label><p className="text-xs text-[#78857E]">Set each item’s due time separately. Leave blank if it has no fixed time.</p></div><Button type="button" size="sm" variant="outline" onClick={() => updateTemplateItems([...templateItemRows, { title: "", dueTime: "" }])}><Plus className="mr-1.5 h-3.5 w-3.5" />Add item</Button></div><div className="space-y-2 rounded-xl border border-[#E1E7E3] bg-[#F8FBF9] p-3">{templateItemRows.length ? templateItemRows.map((row, index) => <div key={index} className="flex items-center gap-2"><Input value={row.title} onChange={event => { const rows = [...templateItemRows]; rows[index] = { ...rows[index], title: event.target.value }; updateTemplateItems(rows); }} placeholder="Morning control checks" className="min-w-0 flex-1" /><Input type="time" aria-label={`Due time for item ${index + 1}`} value={row.dueTime} onChange={event => { const rows = [...templateItemRows]; rows[index] = { ...rows[index], dueTime: event.target.value }; updateTemplateItems(rows); }} className="w-32" /><Button type="button" size="icon" variant="ghost" aria-label={`Remove item ${index + 1}`} onClick={() => updateTemplateItems(templateItemRows.filter((_, itemIndex) => itemIndex !== index))} className="h-9 w-9 text-[#A84237]"><Trash2 className="h-4 w-4" /></Button></div>) : <p className="py-3 text-center text-xs text-[#78857E]">Add the first checklist item.</p>}</div></div></div><DialogFooter><Button onClick={submitTemplate} disabled={templateMutation.isPending || templateUpdateMutation.isPending} className="bg-[#1D5C63] hover:bg-[#164B50]">{editingTemplateId ? "Save changes" : "Save checklist"}</Button></DialogFooter></DialogContent></Dialog><Dialog open={showTaskForm} onOpenChange={setShowTaskForm}><Button variant="outline" className="h-10 border-[#D6E2DE] bg-white" onClick={() => { setPersonalMode(true); setShowTaskForm(true); }}>Personal task</Button><DialogTrigger asChild><Button className="h-10 bg-[#1D5C63] shadow-sm hover:bg-[#164B50]" onClick={() => setPersonalMode(false)}><Plus className="mr-2 h-4 w-4" />Add task</Button></DialogTrigger><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>{personalMode ? "Add a personal reminder" : "Add a daily task"}</DialogTitle><DialogDescription>{personalMode ? "Private to you, unless you are a lead or superuser reviewing personal work." : "Give the team an unambiguous action, owner and deadline."}</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="space-y-2"><Label>Task</Label><Input value={taskForm.title} onChange={event => setTaskForm({ ...taskForm, title: event.target.value })} placeholder="Confirm late-cover driver" /></div><div className="space-y-2"><Label>Notes</Label><Textarea value={taskForm.detail} onChange={event => setTaskForm({ ...taskForm, detail: event.target.value })} placeholder="Context or completion standard…" /></div><div className="rounded-xl border border-[#E1E7E3] bg-[#F8FBF9] p-3"><p className="text-xs font-semibold text-[#3D5149]">Task resource (optional)</p><p className="mt-0.5 text-[11px] text-[#7A8680]">Add the SharePoint or OneDrive document the team needs.</p><div className="mt-2 space-y-2"><Input value={taskForm.resourceUrl} onChange={event => setTaskForm({ ...taskForm, resourceUrl: event.target.value })} placeholder="https://yourcompany.sharepoint.com/..." aria-label="SharePoint or OneDrive URL" /><div className="grid gap-2 sm:grid-cols-2"><Input value={taskForm.resourceTitle} onChange={event => setTaskForm({ ...taskForm, resourceTitle: event.target.value })} placeholder="Resource title" aria-label="Resource title" /><Input value={taskForm.resourceNote} onChange={event => setTaskForm({ ...taskForm, resourceNote: event.target.value })} placeholder="Optional note" aria-label="Optional resource note" /></div></div></div><div className="grid gap-4 sm:grid-cols-3"><div className="space-y-2"><Label>Priority</Label><select value={taskForm.priority} onChange={event => setTaskForm({ ...taskForm, priority: event.target.value as TaskRow["priority"] })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="critical">Critical</option></select></div><div className="space-y-2"><Label>Due time</Label><Input type="time" value={taskForm.dueTime} onChange={event => setTaskForm({ ...taskForm, dueTime: event.target.value })} /></div><div className={`space-y-2 ${personalMode ? "hidden" : ""}`}><Label>Owner (required)</Label><select value={taskForm.assignee} onChange={event => setTaskForm({ ...taskForm, assignee: event.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Choose a staff member…</option>{teamQuery.data?.filter(member => member.status === "active").map(member => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select></div></div></div><DialogFooter><Button onClick={submitTask} disabled={taskMutation.isPending} className="bg-[#1D5C63] hover:bg-[#164B50]">{personalMode ? "Save personal reminder" : "Add task"}</Button></DialogFooter></DialogContent></Dialog></> : null}</div></header>
      ...
      <section className="grid gap-4 md:grid-cols-4"><SummaryCard label="Open actions" value={grouped.action.length} description="Pending or in progress" tone="teal" /><SummaryCard label="Blocked" value={grouped.blocked.length} description={grouped.blocked.length ? "Needs a removal plan" : "No reported blockers"} tone="red" /><SummaryCard label="Missed" value={grouped.missed.length} description={grouped.missed.length ? "Lead review required" : "No missed work"} tone="red" /><SummaryCard label="Completed" value={`${grouped.complete.length}/${visibleTasks.length}`} description="Completion evidenced today" tone="green" /></section>

      {templatesQuery.data?.length ? <section className="mt-6 overflow-hidden rounded-2xl border border-[#E1E7E3] bg-white shadow-[0_16px_36px_-30px_rgba(22,53,45,0.38)]"><div className="flex flex-col gap-3 border-b border-[#EDF0EE] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#E6F0ED] text-[#317168]"><Sparkles className="h-4 w-4" /></div><div><p className="text-sm font-semibold text-[#23302B]">Start from a checklist</p><p className="text-xs text-[#72807A]">Schedule this checklist by day and give every task a named owner.</p></div></div><div className="flex flex-wrap gap-2">{templatesQuery.data.map(template => <div key={template.id} className="flex items-center gap-1"><Button variant="outline" size="sm" disabled={!isManager || weeklyPlannerMutation.isPending} onClick={() => { setPlannerTemplateId(String(template.id)); setShowWeeklyPlanner(true); }}>{template.name}</Button>{isManager ? <><Button variant="ghost" size="icon" className="h-8 w-8 text-[#55736A]" onClick={() => editTemplate(template)} aria-label={`Edit ${template.name}`}><Pencil className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="icon" className="h-8 w-8 text-[#A84237] hover:bg-[#FFF2F0]" onClick={() => deleteTemplate(template)} disabled={templateRemoveMutation.isPending} aria-label={`Delete ${template.name}`}><Trash2 className="h-3.5 w-3.5" /></Button></> : null}</div>)}</div></div></section> : null}

      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]"><div className="overflow-hidden rounded-2xl border border-[#E1E7E3] bg-white shadow-[0_16px_36px_-30px_rgba(22,53,45,0.38)]"><div className="flex items-center justify-between border-b border-[#EDF0EE] px-5 py-4"><div><h2 className="text-base font-semibold text-[#22302B]">{showMine ? "My work queue" : "Today’s task board"}</h2><p className="mt-0.5 text-xs text-[#74807B]">Work requiring active operational attention.</p></div><ClipboardCheck className="h-5 w-5 text-[#78918A]" /></div><div>{tasksQuery.isLoading ? <TaskListSkeleton /> : grouped.action.length ? grouped.action.map(task => <TaskItem key={task.id} task={task} selectedDate={selectedDate} onStatus={status => statusMutation.mutate({ taskId: task.id, status })} pending={statusMutation.isPending} members={teamQuery.data ?? []} latestComment={latestCommentByTask.get(task.id)} />) : <EmptyTasks isManager={isManager} onAdd={() => setShowTaskForm(true)} />}</div>{grouped.missed.length ? <div className="border-t border-[#F2E0C8] bg-[#FFFBF5] px-5 py-4"><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#A25E16]">Missed work requiring action</p><span className="text-xs font-semibold text-[#A25E16]">{grouped.missed.length}</span></div>{grouped.missed.map(task => <div key={task.id} className="border-b border-[#F2E0C8] py-3 last:border-0"><TaskItem task={task} selectedDate={selectedDate} onStatus={status => statusMutation.mutate({ taskId: task.id, status })} pending={statusMutation.isPending} members={teamQuery.data ?? []} latestComment={latestCommentByTask.get(task.id)} compact /><p className="ml-8 mt-1 text-xs text-[#8A5B25]">{task.missedReason || "No reason recorded."} Follow-up: {task.missedFollowUp || "Review required."}</p></div>)}</div> : null}{grouped.complete.length ? <div className="border-t border-[#EDF0EE] bg-[#FBFCFB] px-5 py-4"><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-[#84918B]">Completed</p>{grouped.complete.map(task => <TaskItem key={task.id} task={task} selectedDate={selectedDate} onStatus={status => statusMutation.mutate({ taskId: task.id, status })} pending={statusMutation.isPending} members={teamQuery.data ?? []} latestComment={latestCommentByTask.get(task.id)} compact />)}</div> : null}</div>
        <aside className="space-y-5"><div className="rounded-2xl bg-[#1B3330] p-5 text-white shadow-[0_18px_38px_-22px_rgba(15,48,43,0.75)]"><div className="flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#BBD1C9]">Daily standard</p><Clock3 className="h-4 w-4 text-[#BBD1C9]" /></div><p className="mt-5 text-lg font-medium leading-7">Make the next decision visible, owned and time-bound.</p><div className="mt-5 border-t border-white/15 pt-4 text-xs leading-5 text-[#C7D8D2]">Completion history protects the team as much as it protects the operation.</div></div><div className="rounded-2xl border border-[#F0D8D2] bg-[#FFFAF8] p-5"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-[#7F3D35]">Live exceptions</p><p className="mt-0.5 text-xs text-[#9A6A63]">Open issues assigned to the team.</p></div><CircleAlert className={`h-5 w-5 ${visibleIssues.length ? "text-[#C84E3E]" : "text-[#87A198]"}`} /></div><div className="mt-4 space-y-2">{visibleIssues.length ? visibleIssues.slice(0, 4).map((issue: any) => <div key={issue.id} className="rounded-xl bg-white px-3 py-3"><div className="flex items-start gap-2"><span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${issue.impact === "critical" || issue.impact === "high" ? "bg-[#C84E3E]" : "bg-[#D49A35]"}`} /><div className="min-w-0"><p className="truncate text-xs font-semibold text-[#4A3834]">{issue.title}</p><p className="mt-1 truncate text-[11px] text-[#886E68]">Owner: {teamQuery.data?.find((member: any) => member.id === issue.ownerTeamMemberId)?.displayName || "Unassigned"}</p></div></div></div>) : <p className="rounded-xl bg-white px-3 py-4 text-center text-xs text-[#4C7665]">No open exceptions.</p>}</div>{visibleIssues.length > 4 ? <p className="mt-2 text-[11px] text-[#886E68]">+ {visibleIssues.length - 4} more active exception{visibleIssues.length - 4 === 1 ? "" : "s"}</p> : null}<a href="/issues" className="mt-4 flex items-center justify-between text-xs font-semibold text-[#A84237] hover:underline">Open issue log <span aria-hidden="true">→</span></a></div><div className="rounded-2xl border border-[#E1E7E3] bg-white p-5"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-[#27342F]">Blocked work</p><p className="mt-0.5 text-xs text-[#76827D]">Remove friction early.</p></div><CircleAlert className={`h-5 w-5 ${grouped.blocked.length ? "text-[#C84E3E]" : "text-[#87A198]"}`} /></div><div className="mt-4 space-y-3">{grouped.blocked.length ? grouped.blocked.map(task => <div key={task.id} className="rounded-xl bg-[#FFF5F3] px-3 py-3"><TaskDetail task={task} selectedDate={selectedDate} /><p className="mt-1 line-clamp-1 text-xs text-[#A4574B]">{task.blockedReason || "Action required"}</p></div>) : <p className="rounded-xl bg-[#F3F8F5] px-3 py-4 text-center text-xs text-[#4C7665]">Nothing is blocked right now.</p>}</div></div></aside>
      </section>
    </div>
  );
}
...
function TaskItem({ task, selectedDate, onStatus, pending, compact = false, members = [], latestComment }: { task: TaskRow; selectedDate: string; onStatus: (status: TaskRow["status"]) => void; pending: boolean; compact?: boolean; members?: any[]; latestComment?: any }) {
  const done = task.status === "complete";
  const overdue = Boolean(task.dueAt && task.dueAt < Date.now() && !done);
  return <div className={`flex gap-3 px-5 ${compact ? "py-2.5" : "py-4"} ${!compact ? "border-b border-[#EDF0EE] last:border-0" : ""}`}><button disabled={pending} aria-label={done ? "Reopen task" : "Complete task"} onClick={() => onStatus(done ? "pending" : "complete")} className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-colors ${done ? "border-[#2F7B59] bg-[#2F7B59] text-white" : "border-[#BDC9C4] bg-white hover:border-[#1D5C63]"}`}>{done ? <Check className="h-3 w-3" /> : null}</button><div className="min-w-0 flex-1"><TaskDetail task={task} selectedDate={selectedDate} />{latestComment?.body ? <div className="mt-2 flex items-start gap-1.5 rounded-lg bg-[#F4F8F6] px-2.5 py-2 text-xs text-[#52635C]"><MessageSquareText className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#4C8171]" /><p className="line-clamp-2"><span className="font-semibold text-[#3D574D]">Latest note:</span> {latestComment.body}<span className="ml-1 text-[10px] text-[#84918B]">— {latestComment.actorName || "Team"}{latestComment.createdAt ? `, ${format(new Date(latestComment.createdAt), "d MMM HH:mm")}` : ""}</span></p></div> : null}<div className="mt-2 flex flex-wrap items-center gap-2"><TaskStatusBadge status={task.status} /><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-[0.08em] ring-1 ring-inset ${priorityStyle[task.priority]}`}>{task.priority}</span>{task.assignee ? <span className="inline-flex items-center gap-1.5 text-xs text-[#65726D]"><span className="flex h-4 w-4 items-center justify-center rounded-full text-[8px] font-bold text-white" style={{ backgroundColor: task.assignee.colour }}>{task.assignee.initials}</span>{task.assignee.displayName}</span> : <span className="text-xs text-[#9AA39F]">Unassigned</span>}{task.assistanceTeamMemberIds?.length ? <span className="inline-flex items-center gap-1 rounded-full bg-[#F1F8FB] px-2 py-1 text-[10px] font-semibold text-[#2C6475]">Assistance required: {task.assistanceTeamMemberIds.map(id => members.find(member => member.id === id)?.displayName).filter(Boolean).join(", ") || `${task.assistanceTeamMemberIds.length} colleague${task.assistanceTeamMemberIds.length === 1 ? "" : "s"}`}</span> : null}{done && task.completedByName ? <span className="text-xs font-medium text-[#2F7B59]">Completed by {task.completedByName}</span> : null}{task.originalWorkDate && task.originalWorkDate !== selectedDate ? <span className="text-xs text-[#8A6A3B]">Originally {format(new Date(`${task.originalWorkDate}T12:00:00`), "d MMM")}</span> : null}</div></div><div className="shrink-0 text-right"><p className={`text-xs font-semibold ${overdue ? "text-[#B94336]" : "text-[#65716C]"}`}>{compactTime(task.dueAt)}</p>{overdue ? <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-[#B94336]">Overdue</p> : null}{task.status !== "complete" && task.status !== "in_progress" ? <button onClick={() => onStatus("in_progress")} className="mt-2 text-[11px] font-semibold text-[#31718A] hover:underline">Start</button> : null}</div></div>;
}
