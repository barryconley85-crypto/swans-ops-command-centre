...
export default function Tasks() {
  ...
  const submitTemplate = () => {
    ...
    const payload = { name: templateForm.name.trim(), description: templateForm.description.trim() || undefined, items: items.map(title => ({ title, priority: "normal" })) };
    if (editingTemplateId) templateUpdateMutation.mutate({ id: editingTemplateId, ...payload });
    else templateMutation.mutate(payload);
  };
  const editTemplate = (template: any) => { setEditingTemplateId(template.id); setTemplateForm({ name: template.name || "", description: template.description || "", items: (template.items || []).map((item: any) => item.title).join("\n") }); setShowTemplateForm(true); };
