...
import { Check, ChevronLeft, ChevronRight, CircleAlert, ClipboardCheck, Clock3, FileText, MessageSquareText, Plus, RotateCcw, Sparkles, Trash2 } from "lucide-react";
...
function TaskDetail({ task, selectedDate }: { task: TaskRow; selectedDate: string }) {
  const { user } = useAuth();
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-xl border-0 bg-[#FCFCFA] p-0 shadow-2xl">
        ...
        <div className="border-t border-[#E5E9E6] px-6 py-5">
          <div className="flex items-center justify-between"><p className="text-sm font-semibold text-[#28332F]">Activity</p>{task.completionMode === "lead" && !user?.canManageOperations ? <span className="rounded-full bg-[#F3EEE3] px-2.5 py-1 text-[10px] font-semibold text-[#84662D]">Lead-controlled</span> : <div className="flex gap-1.5"><Button size="sm" variant="outline" onClick={() => setTaskStatus("in_progress")}>Start</Button><Button size="sm" className="bg-[#1D5C63] hover:bg-[#164B50]" onClick={() => setTaskStatus("complete")}><Check className="mr-1 h-3.5 w-3.5" />Complete</Button></div>}</div>
...
export default function Tasks() {
  ...
  const [taskForm, setTaskForm] = useState({ startDate: selectedDate, endDate: selectedDate, title: "", detail: "", priority: "normal" as TaskRow["priority"], dueTime: "", assignee: "", completionMode: "team" as "team" | "lead" });
  const [templateForm, setTemplateForm] = useState({ name: "", description: "", items: "", completionMode: "team" as "team" | "lead" });
  const [templateApplyForm, setTemplateApplyForm] = useState({ startDate: selectedDate, endDate: selectedDate, assignee: "" });
  ...
  const taskMutation = trpc.operations.tasks.create.useMutation({
    onSuccess: async () => { await utils.operations.tasks.list.invalidate({ date: selectedDate }); setShowTaskForm(false); setTaskForm({ startDate: selectedDate, endDate: selectedDate, title: "", detail: "", priority: "normal", dueTime: "", assignee: "", completionMode: "team" }); toast.success("Task run added to the control board."); },
    onError: error => toast.error(error.message),
  });
  const statusMutation = trpc.operations.tasks.updateStatus.useMutation({
  ...
  const templateMutation = trpc.operations.templates.create.useMutation({
    onSuccess: async () => { await templatesQuery.refetch(); setShowTemplateForm(false); setTemplateForm({ name: "", description: "", items: "", completionMode: "team" }); toast.success("Checklist template saved."); },
    onError: error => toast.error(error.message),
  });
  const applyTemplateMutation = trpc.operations.templates.applyTemplate.useMutation({
    onSuccess: async (result) => { await utils.operations.tasks.list.invalidate({ date: selectedDate }); setShowApplyForm(false); toast.success(`${result.created} tasks added across ${result.days} day${result.days === 1 ? "" : "s"}.`); },
    onError: error => toast.error(error.message),
  });
  const removeTaskMutation = trpc.operations.tasks.remove.useMutation({ onSuccess: async () => { await utils.operations.tasks.list.invalidate({ date: selectedDate }); toast.success("Task removed from the board."); }, onError: error => toast.error(error.message) });
  const removeTemplateMutation = trpc.operations.templates.remove.useMutation({ onSuccess: async () => { await templatesQuery.refetch(); toast.success("Checklist template removed."); }, onError: error => toast.error(error.message) });
  ...
  const submitTask = () => {
    ...
    if (taskForm.endDate < taskForm.startDate) return toast.error("Choose an end date on or after the start date.");
    const dates = dateKeysBetween(taskForm.startDate, taskForm.endDate);
    if (dates.length > 21) return toast.error("Choose a task run of up to 21 days.");
    taskMutation.mutate({ workDate: taskForm.startDate, startDate: taskForm.startDate, endDate: taskForm.endDate, title: taskForm.title, detail: taskForm.detail || undefined, priority: taskForm.priority, completionMode: taskForm.completionMode, dueAt: taskForm.dueTime ? new Date(`${taskForm.startDate}T${taskForm.dueTime}:00`).getTime() : undefined, assignedTeamMemberId: taskForm.assignee ? Number(taskForm.assignee) : undefined, repeatDates: dates });
  };
  const submitTemplate = () => {
    ...
    templateMutation.mutate({ name: templateForm.name, description: templateForm.description || undefined, items: items.map(title => ({ title, priority: "normal", completionMode: templateForm.completionMode })) });
  };
  const openTemplateApply = (templateId: number) => { setSelectedTemplateId(templateId); setTemplateApplyForm({ startDate: selectedDate, endDate: selectedDate, assignee: "" }); setShowApplyForm(true); };
  const submitTemplateApply = () => { if (!selectedTemplateId) return; if (templateApplyForm.endDate < templateApplyForm.startDate) return toast.error("Choose an end date on or after the start date."); applyTemplateMutation.mutate({ templateId: selectedTemplateId, startDate: templateApplyForm.startDate, endDate: templateApplyForm.endDate, assignedTeamMemberId: templateApplyForm.assignee ? Number(templateApplyForm.assignee) : undefined }); };
  ...
  return (
    <div className="mx-auto max-w-[1450px] pb-10">
      ...
      {templatesQuery.data?.length ? <section className="mt-6 overflow-hidden rounded-2xl border border-[#E1E7E3] bg-white shadow-[0_16px_36px_-30px_rgba(22,53,45,0.38)]"><div className="flex flex-col gap-3 border-b border-[#EDF0EE] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#E6F0ED] text-[#317168]"><Sparkles className="h-4 w-4" /></div><div><p className="text-sm font-semibold text-[#23302B]">Start from a checklist</p><p className="text-xs text-[#72807A]">Apply an approved routine across one or more days.</p></div></div><div className="flex flex-wrap gap-2">{templatesQuery.data.map(template => <div key={template.id} className="flex items-center gap-1"><Button variant="outline" size="sm" disabled={!isManager || applyTemplateMutation.isPending} onClick={() => openTemplateApply(template.id)}>{template.name}</Button>{user?.workspaceRole === "lead" ? <Button variant="ghost" size="icon" aria-label={`Delete ${template.name} template`} className="h-8 w-8 text-[#A54A40] hover:bg-[#FFF3F1]" onClick={() => { if (window.confirm(`Delete the ${template.name} checklist template? Existing tasks will stay.`)) removeTemplateMutation.mutate({ templateId: template.id }); }}><Trash2 className="h-3.5 w-3.5" /></Button> : null}</div>)}</div></div></section> : null}
      <Dialog open={showApplyForm} onOpenChange={setShowApplyForm}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Apply checklist to a date range</DialogTitle><DialogDescription>Create a fresh task run for every day in the selected range. Existing tasks are not overwritten.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>From</Label><Input type="date" value={templateApplyForm.startDate} onChange={event => setTemplateApplyForm({ ...templateApplyForm, startDate: event.target.value, endDate: templateApplyForm.endDate < event.target.value ? event.target.value : templateApplyForm.endDate })} /></div><div className="space-y-2"><Label>To</Label><Input type="date" min={templateApplyForm.startDate} value={templateApplyForm.endDate} onChange={event => setTemplateApplyForm({ ...templateApplyForm, endDate: event.target.value })} /></div></div><div className="space-y-2"><Label>Assign all items to</Label><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={templateApplyForm.assignee} onChange={event => setTemplateApplyForm({ ...templateApplyForm, assignee: event.target.value })}><option value="">Keep template owners / unassigned</option>{teamQuery.data?.filter(member => member.status === "active").map(member => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select><p className="text-xs text-[#77827E]">Up to 21 days. Assigning one person here overrides template item owners.</p></div></div><DialogFooter><Button onClick={submitTemplateApply} disabled={applyTemplateMutation.isPending} className="bg-[#1D5C63] hover:bg-[#164B50]">Apply checklist run</Button></DialogFooter></DialogContent></Dialog>

      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]"><div className="overflow-hidden rounded-2xl border border-[#E1E7E3] bg-white shadow-[0_16px_36px_-30px_rgba(22,53,45,0.38)]"><div className="flex items-center justify-between border-b border-[#EDF0EE] px-5 py-4"><div><h2 className="text-base font-semibold text-[#22302B]">{showMine ? "My work queue" : "Today’s task board"}</h2><p className="mt-0.5 text-xs text-[#74807B]">Work requiring active operational attention.</p></div><ClipboardCheck className="h-5 w-5 text-[#78918A]" /></div><div>{tasksQuery.isLoading ? <TaskListSkeleton /> : grouped.action.length ? grouped.action.map(task => <TaskItem key={task.id} task={task} selectedDate={selectedDate} onStatus={status => statusMutation.mutate({ taskId: task.id, status })} onDelete={isManager ? () => { if (window.confirm(`Remove “${task.title}” from this day?`)) removeTaskMutation.mutate({ taskId: task.id }); } : undefined} pending={statusMutation.isPending} />) : <EmptyTasks isManager={isManager} onAdd={() => setShowTaskForm(true)} />}</div>{grouped.complete.length ? <div className="border-t border-[#EDF0EE] bg-[#FBFCFB] px-5 py-4"><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-[#84918B]">Completed</p>{grouped.complete.map(task => <TaskItem key={task.id} task={task} selectedDate={selectedDate} onStatus={status => statusMutation.mutate({ taskId: task.id, status })} onDelete={isManager ? () => { if (window.confirm(`Remove completed task “${task.title}”? Only the lead can remove completed evidence.`)) removeTaskMutation.mutate({ taskId: task.id }); } : undefined} pending={statusMutation.isPending} compact />)}</div> : null}</div>
...
function TaskItem({ task, selectedDate, onStatus, onDelete, pending, compact = false }: { task: TaskRow; selectedDate: string; onStatus: (status: TaskRow["status"]) => void; onDelete?: () => void; pending: boolean; compact?: boolean }) {
  const done = task.status === "complete";
  return <div className={`flex gap-3 px-5 ${compact ? "py-2.5" : "py-4"} ${!compact ? "border-b border-[#EDF0EE] last:border-0" : ""}`}><button disabled={pending} aria-label={done ? "Reopen task" : "Complete task"} onClick={() => onStatus(done ? "pending" : "complete")} className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-colors ${done ? "border-[#2F7B59] bg-[#2F7B59] text-white" : "border-[#BDC9C4] bg-white hover:border-[#1D5C63]"}`}>{done ? <Check className="h-3 w-3" /> : null}</button><div className="min-w-0 flex-1"><TaskDetail task={task} selectedDate={selectedDate} /><div className="mt-2 flex flex-wrap items-center gap-2"><TaskStatusBadge status={task.status} /><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-[0.08em] ring-1 ring-inset ${priorityStyle[task.priority]}`}>{task.priority}</span>{task.assignee ? <span className="inline-flex items-center gap-1.5 text-xs text-[#65726D]"><span className="flex h-4 w-4 items-center justify-center rounded-full text-[8px] font-bold text-white" style={{ backgroundColor: task.assignee.colour }}>{task.assignee.initials}</span>{task.assignee.displayName}</span> : <span className="text-xs text-[#9AA39F]">Unassigned</span>}{done && task.completedByName ? <span className="text-xs font-medium text-[#2F7B59]">Completed by {task.completedByName}</span> : null}</div></div><div className="flex shrink-0 flex-col items-end text-right"><p className={`text-xs font-semibold ${task.dueAt && task.dueAt < Date.now() && !done ? "text-[#B94336]" : "text-[#65716C]"}`}>{compactTime(task.dueAt)}</p>{task.status !== "complete" && task.status !== "in_progress" ? <button onClick={() => onStatus("in_progress")} className="mt-2 text-[11px] font-semibold text-[#31718A] hover:underline">Start</button> : null}{onDelete ? <button aria-label={`Remove ${task.title}`} onClick={onDelete} className="mt-2 inline-flex items-center gap-1 text-[11px] font-semibold text-[#A54A40] hover:underline"><Trash2 className="h-3 w-3" />Remove</button> : null}</div></div>;
}
