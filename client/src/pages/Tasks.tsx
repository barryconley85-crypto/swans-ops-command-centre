      ...
      <DialogContent className="max-w-xl border-0 bg-[#FCFCFA] p-0 shadow-2xl">
        ...
        <div className="border-t border-[#E5E9E6] px-6 py-5">
          ...
          <div className="mt-4 max-h-40 space-y-3 overflow-y-auto pr-1">
            {activity.data?.length ? activity.data.map(item => <div key={item.id} className="rounded-lg bg-[#F4F6F4] px-3 py-2.5"><p className="text-xs font-semibold capitalize text-[#45504C]">{labelForStatus(item.action)} <span className="ml-1 font-normal text-[#87918D]">{format(new Date(item.createdAt), "d MMM, HH:mm")}</span></p>{item.body ? <p className="mt-1 text-xs leading-5 text-[#65706C]">{item.body}</p> : null}</div>) : <p className="py-3 text-center text-xs text-[#8C9692]">No activity yet.</p>}
          </div>
...
export default function Tasks() {
  ...
  const taskMutation = trpc.operations.tasks.create.useMutation({
    onSuccess: async () => { await utils.operations.tasks.list.invalidate({ date: selectedDate }); setShowTaskForm(false); setTaskForm({ title: "", detail: "", priority: "normal", dueTime: "", assignee: "" }); toast.success("Task added to today’s control board."); },
    onError: error => toast.error(error.message),
  });
  const statusMutation = trpc.operations.tasks.updateStatus.useMutation({
  ...
  const applyTemplateMutation = trpc.operations.templates.applyTemplate.useMutation({
    onSuccess: async (result) => { await utils.operations.tasks.list.invalidate({ date: selectedDate }); toast.success(`${result.created} tasks added to ${dateTitle(selectedDate)}.`); },
    onError: error => toast.error(error.message),
  });
  ...
  const submitTask = () => {
    ...
    taskMutation.mutate({ workDate: selectedDate, title: taskForm.title, detail: taskForm.detail || undefined, priority: taskForm.priority, dueAt: taskForm.dueTime ? new Date(`${selectedDate}T${taskForm.dueTime}:00`).getTime() : undefined, assignedTeamMemberId: taskForm.assignee ? Number(taskForm.assignee) : undefined });
  };
