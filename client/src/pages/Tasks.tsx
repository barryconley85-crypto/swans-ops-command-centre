...
<DialogContent className="max-w-xl border-0 bg-[#FCFCFA] p-0 shadow-2xl">
        <div className="border-b border-[#E5E9E6] px-6 pb-5 pt-6">
          <div className="mb-4 flex items-start justify-between gap-3"><div className="flex items-center gap-2"><TaskStatusBadge status={task.status} /><Badge className={`border-0 text-[10px] uppercase tracking-[0.12em] ${priorityStyle[task.priority]}`}>{task.priority}</Badge></div>{canManageTasks && !editing ? <div className="flex flex-wrap justify-end gap-1.5"><Button size="sm" variant="outline" onClick={() => { setDraft({ workDate: task.workDate, title: task.title, detail: task.detail || "", priority: task.priority, dueTime: task.dueAt ? format(new Date(task.dueAt), "HH:mm") : "", assignedTeamMemberId: task.assignedTeamMemberId ? String(task.assignedTeamMemberId) : "" }); setEditing(true); }}><Pencil className="mr-1.5 h-3.5 w-3.5" />Edit</Button><Button size="sm" variant="outline" disabled={visibilityMutation.isPending} onClick={() => visibilityMutation.mutate({ taskId: task.id, isPersonal: !task.isPersonal })}>{task.isPersonal ? "Make shared" : "Make personal"}</Button></div> : null}</div>
