...
export default function Tasks() {
  ...
  const templateMutation = trpc.operations.templates.create.useMutation({
    onSuccess: async () => { await templatesQuery.refetch(); setShowTemplateForm(false); setEditingTemplateId(null); setTemplateForm({ name: "", description: "", items: "" }); toast.success("Checklist template saved."); },
    onError: error => toast.error(error.message),
  });
  const templateUpdateMutation = trpc.operations.templates.update.useMutation({
    onSuccess: async () => { await templatesQuery.refetch(); setShowTemplateForm(false); setEditingTemplateId(null); setTemplateForm({ name: "", description: "", items: "" }); toast.success("Checklist template updated."); },
    onError: error => toast.error(error.message),
  });
  ...
  const submitTemplate = () => {
    ...
    const payload = { name: templateForm.name.trim(), description: templateForm.description.trim() || undefined, items };
    if (editingTemplateId) templateUpdateMutation.mutate({ id: editingTemplateId, ...payload });
    else templateMutation.mutate(payload);
  };
  const editTemplate = (template: any) => { setEditingTemplateId(template.id); setTemplateForm({ name: template.name || "", description: template.description || "", items: (template.items || []).map((item: any) => `${item.dueTime ? `${item.dueTime} | ` : ""}${item.title}`).join("\n") }); setShowTemplateForm(true); };
