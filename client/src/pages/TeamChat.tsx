...
export default function TeamChat() {
  const { user } = useAuth();
  const canChat = Boolean(user?.canUseOnCall);
  ...
  return <div className="mx-auto max-w-[1120px] pb-10">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_270px]">
      <Panel className="flex min-h-[610px] flex-col">
        ...
        <div className="border-t border-[#E6ECE8] bg-white p-4">{canChat ? <><div className="flex gap-3"><Textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") submit(); }} placeholder="Share an operational update…" className="min-h-[76px] resize-none border-[#DCE6E0] bg-[#FCFDFC] text-sm" maxLength={1000} /><Button onClick={submit} disabled={!draft.trim() || send.isPending} className="h-auto shrink-0 bg-[#1D5C63] px-4 hover:bg-[#164B50]" aria-label="Send message"><Send className="h-4 w-4" /></Button></div><p className="mt-2 text-[11px] text-[#83908A]">Press Ctrl/⌘ + Enter to send. Avoid personal, customer or payment information.</p></> : <p className="rounded-xl bg-[#F4F7F5] px-3 py-3 text-xs leading-5 text-[#6B7872]">Your Viewer level is read-only. Ask the operations lead if you need to share an update.</p>}</div>
      </Panel>
