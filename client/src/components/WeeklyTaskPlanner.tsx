    ...
    <DialogContent className="w-[calc(100vw-1rem)] max-w-[1400px] min-w-0 max-h-[92vh] overflow-hidden border-0 bg-[#FCFCFA] p-0 shadow-2xl sm:max-w-[1400px]">
      ...
      <div className="min-w-0 space-y-4 overflow-y-auto px-6 py-5">
        ...
        {template ? <>
          ...
          <div className="min-w-0 max-w-full overflow-x-auto rounded-xl border border-[#E2E8E4]">
            <div className="min-w-[910px]">
              <div className="grid grid-cols-[210px_repeat(7,minmax(100px,1fr))] border-b border-[#E6ECE8] bg-[#F8FAF8]"><div className="px-4 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-[#7A8780]">Task from checklist</div>{days.map(day => <div key={day.toISOString()} className="border-l border-[#E6ECE8] px-2 py-3"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#7A8780]">{format(day, "EEE")}</p><p className="mt-1 text-sm font-semibold text-[#2C3934]">{format(day, "d MMM")}</p></div>)}</div>
              {items.map((item: any, itemIndex: number) => { const itemKey = String(item.id ?? itemIndex); return <div key={itemKey} className="grid grid-cols-[210px_repeat(7,minmax(100px,1fr))] border-b border-[#EEF2EF] last:border-0"><div className="px-4 py-3"><p className="text-sm font-semibold text-[#2E3D35]">{item.title}</p>{item.detail ? <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-[#78857E]">{item.detail}</p> : null}</div>{days.map(day => <div key={day.toISOString()} className="border-l border-[#EEF2EF] p-1.5"><select aria-label={`${item.title} on ${format(day, "EEEE d MMMM")}`} value={owners[`${itemKey}:${localDateKey(day)}`] ?? ""} onChange={event => setOwner(itemKey, day, event.target.value)} className="h-9 w-full min-w-0 truncate rounded-md border border-[#DCE5DF] bg-white px-1.5 text-[11px] text-[#405249]"><option value="">Not scheduled</option>{members.map(member => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select></div>)}</div>; })}
            </div>
          </div>
